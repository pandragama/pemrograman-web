...
if (isset($_POST["submit"])) {
  ...
  if ($error_message === "") {
    ...
    $query .= "nim = '$nim', name = '$name', birth_city = '$birth_city', ";
    $query .= "birth_date = '$birth_date_full', faculty = '$faculty', department = '$department', gpa = $gpa ";
    $query .= "WHERE nim = '$nim';";

    $result = mysqli_query($connection, $query);

    if ($result) {
      $message = "Data mahasiswa dengan nama = \"<b>$name</b>\" berhasil diubah";
  ...
  $nim = htmlentities(strip_tags(trim($data["nim"])));
  $name = htmlentities(strip_tags(trim($data["name"])));
  $birth_city = htmlentities(strip_tags(trim($data["birth_city"])));
  $faculty = htmlentities(strip_tags(trim($data["faculty"])));
  $department = htmlentities(strip_tags(trim($data["department"])));
  $gpa = htmlentities(strip_tags(trim($data["gpa"])));
  $birth = explode("-", $data["birth_date"]);
  $birth_date = htmlentities(strip_tags(trim($birth[2])));
  $birth_month = htmlentities(strip_tags(trim($birth[1])));
  $birth_year = htmlentities(strip_tags(trim($birth[0])));
    ...
    <form id="form_mahasiswa" action="student_edit.php" method="post">
      <fieldset>
        ...
        <p>
          ...
          <input type="text" id="nim" value="<?php echo $nim ?>" disabled> (8 digit angka)
      ...
      <br>
      <p>
        <input type="submit" name="submit" value="Simpan Perubahan Data">
      </p>
    </form>
  </div>
</body>
